...
function Installer (where, dryrun, args, opts) {
  ...
  if (!opts) opts = {}
  //字符串 表示当前运行install命令的目录 
  this.where = where
  //如果为true表明你不需要让npm做出任何更新并且只报告完成了什么
  this.dryrun = dryrun
  //数组，表示运行install命令传进来的参数
  this.args = args
  ...
  //读取配置文件 package-lock 判断是否自动生成package-lock.json文件
  this.autoPrune = npm.config.get('package-lock')
  //当是dev或者development时，不带任何参数运行局部npm install，
  //只会有devDependencies(和他们的依赖)会被安装，
  //当是prod或者production是，无参运行npm install，
  //只有non-devDependencies(和他们的依赖)会被安装 
  ...
  //如果为true，则不进行安装动作，
  //只是将安装的内容写进package.json的dependencies里面
...
//判断是否是全局安装，（this.global 为true 表示是全局安装有 -g 参数），
//如果是则调用readGlobalPackageData读取全局目录下的node_modules文件树，
//如果不是则调用readLocalPackageData读取当前项目目录下的node_modules文件树
...
Installer.prototype.loadIdealTree = function (cb) {
  ...
  chain([
    ...
    //调用loadShrinkwrap函数 ，该函数会依次读取
    //npm-shrinkwrap.json、package-lock.json、package.json ，
    //确定完整的依赖树，注意这时候的依赖树只是逻辑上的依赖树
    ...
    //调用loadAllDepsIntoIdealTree函数 遍历上一步得到的idealTree，
    //添加任何缺少的依赖项，在不破坏原来的module的情况下，依赖项会尽可能的放到顶层。
...
Installer.prototype.executeActions = function (cb) {
  ...
  // 执行各种action，具体的action定义在lib/install/action.js里
  steps.push(
